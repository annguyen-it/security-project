...
def gen_key(master_key):
    round_keys = text2matrix(master_key)

    for i in range(4, 4 * 11):
        round_keys.append([])
        if i % 4 == 0:
            byte = round_keys[i - 4][0] ^ Sbox[round_keys[i - 1][1]] ^ Rcon[i // 4]
...
if __name__ == '__main__':
    ...
    keys = gen_key(key)
    res = encrypt(0x0123456789abcdeffedcba9876543210, keys)
    print(hex(decrypt(res, keys)))
